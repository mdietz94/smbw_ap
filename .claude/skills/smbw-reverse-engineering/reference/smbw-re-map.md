...
| `+0x1c3e560` | `resolvePaneContent(this, int* paneId, _, _, msgLabelObj)` — per detail-pane, builds the **msbt label** for the SELECTED item: case 0 = name pane (badge gparam `+0x40` → `msgLabelObj+0x698`, file "GameMsg/Name_Badge"), case 1 = desc (`+0x48` → `+0x768`, "GameMsg/BadgeInfo"), case 2 = price int → `+0xc`. Selected badge id @ `this+0x6ac` (s32), type @ `+0x6a8`. **Hooked** (`BadgeShopPaneResolve`): arms the AP shop-text one-shot for the selected badge so the global resolver override is shop-scoped (the equip menu is a different screen → never calls this). Prologue stp/str/stp/mov/ldr SAFE | CONFIRMED static |
| `+0x250dec` | `FUN_7100250dec(x0 store, x1 OUT{char16_t*;u32 len;u64 id}, x2 file C-str, x3 label-holder)` — **game-wide msbt label→text resolver**. file = `(char*)x2` ("GameMsg/Name_Badge" / "GameMsg/BadgeInfo" / "LayoutMsgRef/…" / …). Returns status byte (0=found, caller `cbnz w0` bails on nonzero); OUT consumed synchronously by `+0x4eee20` (memcpy'd out — a static char16_t buffer is safe). **Hooked** (`MsbtLabelResolve`): gated by `probe::tryServeBadgeShopText`. **⚠ Live correction (2026-06-11): `*(char**)x3` is NOT a usable per-badge id here** — the prior "BadgeId%02d" claim is WRONG (real labels seen: "ExActReturnJump", "Badge_PowerUp", layout pane names). Identity is instead the **arm** (selected badge id from `resolvePaneContent`, `this+0x6ac`) + **file**: the selected badge's description is the *single* `GameMsg/BadgeInfo` lookup per shop render, so we serve that one keyed by the armed id and ignore the label. (`Name_Badge` is looked up several times/render → ambiguous, so we override the description, not the name.) The arm expires after ~16 frames (frame-stamp via `playerTickLatch`) so it can't bleed into the equip menu. Prologue sub/stp/str/stp/stp SAFE | CONFIRMED (live) |
