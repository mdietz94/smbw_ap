...
namespace gui {
    ...
    void ActorBaseCtor::Callback(engine::actor::ActorBase* thisPtr, void* x1)
    {
        ...
        // M1.3 investigation outcome: the ctor at NSO 0x231204 only fires for
        // a handful of early system actors on this build — game-level actors
        // (ObjectGoalPole, ItemWonderFinishWonderSead, ...) construct through
        // a different path that we haven't identified. Runtime vtable
        // discovery via this hook isn't viable; using Ghidra static analysis
        // instead. Leaving the call intact so the probe TU still links;
        // it returns early because no target matches.
        smbwap::probe::onActorConstructed(reinterpret_cast<engine::actor::IActor*>(thisPtr), x1);
    }
