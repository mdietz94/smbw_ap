...
PopTracker access rules and writes them into a checkout of the tracker:
  ...
  * `scripts/logic/smbw_generated_logic.lua` -- one Lua function per region
    ...
    start (`smbw_world_active`), each active world additionally needs its
...
# Open-world `world_unlock_items` (default ON): each active world additionally
# needs its own "W<n> Unlock" AP item, exactly one of which is precollected.
# The tracker code for these comes from the tracker's own item_mapping.lua, so
# the gate is only emitted for worlds whose Unlock item that map already knows
...
def main():
    ...
    for region in order:
        ...
        if ws is not None:
            ...
            terms = [f"smbw_world_active({ws})"]
            if open_unlock_item(ws) in name2code:
                terms.append(f"smbw_world_unlocked({ws})")
            if open_req != "true":
...
# Emitters
# --------------------------------------------------------------------------- #
def write_lua(tracker, regions, order, region_expr, loc_rules, name2code=None):
    L = []
    ...
    L.append('-- open_world (0/1), open_world_active (list of active world numbers),')
    L.append("-- palaces_required and open_world_unlock_items (0/1).  When off / unset,")
    L.append("-- the standard linear-spine logic runs.")
    ...
    L.append("end")
    # Open-world world-unlock items: map each world to its tracker code, but
    # only for worlds the tracker's item_mapping.lua actually knows.
    unlock_codes = {n: (name2code or {}).get(open_unlock_item(n))
                    for n in range(1, 7)}
    unlock_codes = {n: c for n, c in unlock_codes.items() if c}
    L.append("-- Open-world world-unlock items (slot_data open_world_unlock_items):")
    L.append("-- each active world needs its own \"W<n> Unlock\" AP item; exactly one is")
    L.append("-- precollected at generation.  Returns true when the option is off (or the")
    L.append("-- world has no mapped Unlock code) so older seeds behave as before.")
    L.append("local SMBW_UNLOCK_CODE = {"
             + ", ".join(f'[{n}] = "{c}"' for n, c in sorted(unlock_codes.items()))
             + "}")
    L.append("function smbw_world_unlocked(n)")
    L.append("    if not (SLOT_DATA and (SLOT_DATA.open_world_unlock_items == 1")
    L.append("            or SLOT_DATA.open_world_unlock_items == true)) then return true end")
