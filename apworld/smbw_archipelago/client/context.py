...
class SMBWContext(CommonContext):
    ...
    def __init__(
        ...
        # Badge-specific sub-toggle.  When False, BADGE-kind entry gates
        # never arm (and any in-flight badge kill self-stops within one
        # countdown tick via ``_gate_check_stop``) -- but the final-Bowser
        # ROYAL_SEEDS gate is unaffected.  Master ``entry_gating_enabled``
        # still wins (off there disables everything).  Set from slot_data
        # ``badge_level_entry_gating`` and live-toggleable via the
        # ``/badge_gate_kills`` command.
        self.badge_entry_gating_enabled: bool = True
    ...
    async def _handle_ap_package(self, cmd: str, args: dict) -> None:
        if cmd == "Connected":
            ...
            # Badge-specific sub-toggle (default ON; the Royal-Seed/Bowser
            # gate is governed only by ``level_entry_gating`` above).
            beg = bool(slot_data.get("badge_level_entry_gating", True))
