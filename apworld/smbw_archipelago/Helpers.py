...
def is_item_enabled(multiworld: MultiWorld, player: int, item: SMBWonderItem) -> bool:
    hook_result = before_is_item_enabled(multiworld, player, item)
    if hook_result is not None:
        return hook_result

    return _is_smbwonderobject_enabled(multiworld, player, item)

def is_location_enabled(multiworld: MultiWorld, player: int, location: SMBWonderLocation) -> bool:
    hook_result = before_is_location_enabled(multiworld, player, location)
    if hook_result is not None:
        return hook_result

    return _is_smbwonderobject_enabled(multiworld, player, location)

def _is_smbwonderobject_enabled(multiworld: MultiWorld, player: int, object: any) -> bool:
    enabled = True
...
def get_items_with_value(world: World, multiworld: MultiWorld, value: str, player: Optional[int] = None, force: bool = False) -> dict[str, int]:
    ...
    if not hasattr(world, 'item_values'):
        world.item_values = {}
