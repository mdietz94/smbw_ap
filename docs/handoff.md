...
| `koopajr_result` | palace boss fight result (win OR loss) | `stage_info` (identifies the palace), **`battle_result`** (True=won/Royal Seed earned, False=died), `koopajr_final_stage`, **`koopajr_step_info`** (array of per-phase structs with damage count + time), `koopajr_total_time`, `koopajr_challenge_count`, `badge_id_array`. **An AP Royal Seed check fires only when `battle_result == True`.** ✅ Loss case captured 2026-05-20 (Pipe-Rock Plateau Palace, stage_key=2308078743); win case TBD. |
